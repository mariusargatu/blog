...
faq:
  ...
  - q: What is Cohen's kappa and what counts as a good agreement score?
    a: >-
      ...
      Landis-Koch bands a judge-versus-expert kappa of 0.21 lands in "fair,"
      barely above chance, and a common working bar is kappa at or above 0.6
      before you let a judge automate a metric at all.
...
4. Is it gameable — will a longer or surer answer score higher without being better?
...
Run the lanes and the shape is the argument. Seven of the ten rows are rules: scope, the write gate, correctness against the oracle, the refusal. They gate hard and fail closed, and in [Atlas](https://github.com/mariusargatu/Atlas), the example support agent this series is built around, they are concrete graders the suite resolves by name, the same domain checks the agent itself is bound by, so the eval cannot drift more lenient than the runtime. Only three rows go to the model judge, and every one of them is a question of taste no schema can hold. That ratio is the discipline: spend the determinism everywhere it reaches, and pay for a calibrated judge only where it does not.
...
**A support agent's LLM judge rated faithfulness in the low nineties for three months. When a subject-matter expert hand-graded fifty answers blind, Cohen's kappa between judge and expert came back at 0.21 — "fair," barely above chance. An unchecked judge is not a metric; it is a vibe with a decimal point.**

For three months the dashboard was green. The judge scoring Atlas's document answers was the obvious default: same model family as the agent, same vendor, same SDK, one config line to turn on. It rated faithfulness and helpfulness on every answer the support agent produced, and the line went up and to the right and stayed there. Faithfulness in the low nineties. Helpfulness higher. The kind of chart you put in a board deck.
...
Cohen's kappa came back at 0.21.

That is the number that measures how much two raters agree once you subtract the agreement you would get from luck alone. Zero is chance. One is perfect. On the bands [Landis and Koch](https://www.jstor.org/stable/2529310) published in 1977, 0.21 lands at the bottom of "fair," about a fifth of the distance from chance to perfect, and reads as barely awake. The judge and the expert were agreeing only a little more often than luck alone would manage, dressed up as a metric.

Read the disagreements and the pattern was ugly and specific. The judge loved long, fluent, confident answers, the ones written in its own family's house style, and it loved them whether or not they were true. It waved through the exact failure this series is built around: the customer still on an older, legacy plan who asks about the data cap and gets a grounded sounding, beautifully phrased, entirely false answer, because the retrieved page describes the current plan and not theirs. Faithful to the page. Wrong about the world. The judge scored it a nine.
...
**Here is the part that makes this worth getting right instead of giving up. The problem in the three green months that opened this article is not that the judge was a model. It is that the judge was a model nobody had calibrated.**
...
3. A **human labeled calibration set**, one hundred to three hundred cases, big enough for a kappa with an interval you can stand behind and small enough for experts to maintain.
...
**Every metric here inherits its meaning from the job it judges, and the judge that computes the subjective ones is itself just another model — one layer of defense, anchored to the human labeled calibration set, that never replaces the rule based source of truth that actually stops a confident falsehood.**
...
Cohen's kappa measures how much two raters agree once you subtract the agreement luck alone would produce: zero is chance, one is perfect. On the Landis-Koch bands a judge-versus-expert kappa of 0.21 lands in "fair," barely above chance, and a common working bar is kappa at or above 0.6 before you let a judge automate a metric at all.
