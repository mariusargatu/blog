...
faq:
  ...
  - q: How many items does an LLM eval need?
    a: >-
      ...
      to a round number someone liked. With the per item spread in this
      article's cold open, resolving a three point gap required 962 items; the
      hundred item suite could only resolve nine. An underpowered eval returns
      "no difference detectable," which is not "no difference."
...
**A 3-point gap on a 100-item eval, 84% against 81%, is statistically indistinguishable from noise: the Wilson 95% intervals overlap and a paired test returns p = 0.55. On a sample this size, "B is worse" is a claim the data cannot support.**
...
**When a release gates on an eval metric, gate on the lower bound of the confidence interval, never the point estimate. A candidate scoring 0.81 against a 0.80 bar fails an interval-floor gate (Wilson lower bound 0.722), so "we cannot tell yet" fails closed instead of shipping on optimism.**
...
Size the suite to the smallest effect that would change a decision, not to a round number someone liked. With the per item spread in this article's cold open, resolving a three point gap required 962 items; the hundred item suite could only resolve nine. An underpowered eval returns "no difference detectable," which is not "no difference."
