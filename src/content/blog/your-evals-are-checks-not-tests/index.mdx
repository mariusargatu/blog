...
topic: GenAI
ctaNote: >-
  Air Canada paid for an answer its evals called faithful. If you are shipping an
  LLM, RAG, or agentic system and your dashboard is green, this is the gap I get
  hired to close.
series:
...
The first is a claims-against-canonical-facts validator, the production sibling of Pattern 1. Where Pattern 1 runs in CI against a fixed question set, the validator runs in production against every actual customer response. The goal is that any claim about refund mechanics that disagrees with the canonical facts, or any claim that introduces a fact the canonical source doesn't contain, blocks the response.

Say plainly what a first version of that actually is, because the full goal is harder than the sentence makes it sound. A shippable v1 is cue based: it carries a list of the phrasings that would be dangerous if they were false, and when one of them appears, it asks the canonical facts whether this customer's case contradicts it. That catches the phrasings on the list. It does not catch an arbitrary paraphrase, and it does not catch a number that is merely wrong. Worth stating out loud in the design doc rather than discovering it in an incident review, because a validator you believe is total and is really partial is worse than one you know is partial.
...
A small team can ship the first version of a validator pipeline in three to four weeks. The largest investment is in the claims-against-canonical-facts validator, and most of that investment goes into being precise about how much the first version really covers. Maintenance is ongoing but bounded, and it is mostly the cue list growing as production shows you the phrasings you did not think of, which is Pattern 5 feeding Pattern 2.
...
Next in the series: [The System Under Test](/blog/the-system-under-test/) builds the authenticated broadband support agent that the rest of these patterns are tested against, named part by part by how each one breaks. Watch for the oracle. In a real system it stops being a policy PDF and becomes code, the customer's account joined to the catalog, the one place the truth actually lives. Patterns 1 through 4 are executable there, in [Atlas](https://github.com/mariusargatu/Atlas), the runnable reference system the series is built against. Pattern 5 is not, and the reason is worth saying rather than papering over: Atlas is hermetic and has no production traffic, so there are no real transcripts in it to read. The cheapest pattern in this article is the one a reference repository cannot hand you. That one you run against your own system.
