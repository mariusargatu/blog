...
faq:
  ...
  - q: How does a confirmation gate stop an agent from making an unauthorized change?
    a: >-
      ...
      irreversible happens until a person confirms it.
---
...
**Atlas is a single authenticated support agent for a broadband provider, built on LangGraph and MCP. It does three things: answers plan and policy questions from help documents, reads the signed-in customer's own account, and makes changes to that account. Each is a distinct testing problem: retrieval, a scoped read, and a consequential write.** The customer is signed in, which means the system knows whose account this is from the session, never from anything the customer types into the box. That one fact will come back over and over, so hold onto it.
...
One MCP server, and the only one that changes a real customer's service: `change_plan`, `add_addon`, `remove_addon`, `reset_modem`, `open_ticket`, `book_engineer`. This is the highest consequence surface in the system, and almost everything genuinely hard about testing agents lives on it.
...
**Four invariants make every test in the series mean something: nothing external is real in CI, the model is recorded and replayed, every turn is traced from the first commit, and the customer's identity comes from the authenticated session, never from the model. Pull any one out and the tests stop meaning anything.** They are structural, not polish.
...
**[The series](/blog/series/evals-are-checks-not-tests/) moves left to right across this map, roughly in the order you would build the testing: define correctness with a golden dataset, choose metrics and calibrate the LLM judge that scores them, make the numbers honest with statistics, then test retrieval, trajectory, simulation, and security surface by surface before taking the whole thing to production.**
