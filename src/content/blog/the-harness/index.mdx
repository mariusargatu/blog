...
faq:
  ...
  - q: What is the difference between an agent harness and an eval harness?
    a: >-
      ...
      production: a LangGraph state graph, MCP tools, a checkpointer, and a
      runtime. The eval harness measures whether the first one works, running
      seeded tasks in CI and grading them against an oracle. Same language, the
  ...
  - q: What is model drift and how do you catch it when the model is replayed?
    a: >-
      ...
      green on a stale snapshot. The drift lane reruns the pinned agent against
      a fresh snapshot and diffs what the turn decided, its tools, guards, and
      outcome, read structurally from the trace, not its prose.
---
...
**The eval harness is the second machine: it runs seeded tasks at scale, drives the agent, captures each trace, grades outputs against an oracle, and reports a pass rate, not a verdict. It is built from three separated roles (planner, generator, evaluator) and a cheapest-first grader stack.**

The eval harness is the second machine, built from the same parts as the first but read differently. It lives in its own package — an eval toolkit at `testing/harness/evals/evalkit` — that the agent harness may never import. The shape is small on purpose: three roles, one grader stack, one rate.
...
**This article is the foundation layer of the series: the reproducible test rig and tracing substrate that pins the model, fakes the backends, and records every trace so the downstream numbers land on something stable. The cases from "The Golden Dataset" execute in it, the judge from "Metrics and the Lying Judge" runs through its recorded gateway, and the retrieval and agent articles assert on its traces.** It is the part you own, backed by a runnable reference system ([Atlas](https://github.com/mariusargatu/Atlas)).
...
The agent harness is the scaffolding that lets the model act in production: a LangGraph state graph, MCP tools, a checkpointer, and a runtime. The eval harness measures whether the first one works, running seeded tasks in CI and grading them against an oracle. Same language, the trace; opposite jobs, like a production server versus a build system.

### What is model drift and how do you catch it when the model is replayed?

Model drift is a provider changing the model behind a stable version string, so REPLAY returns last quarter's response and the suite stays green on a stale snapshot. The drift lane reruns the pinned agent against a fresh snapshot and diffs what the turn decided, its tools, guards, and outcome, read structurally from the trace, not its prose.
